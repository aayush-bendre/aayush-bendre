...
x , y, k11, k12, k13, k21, k22, k23, lb = sp.symbols('x y k11 k12 k13 k21 k22 k23 lb')
...
dldx = sp.Derivative(L,x).doit()
dldy = sp.Derivative(L,y).doit()
L2 = np.array([[sp.Derivative(dldx,x).doit(),sp.Derivative(dldx,y).doit()],[sp.Derivative(dldy,x).doit(),sp.Derivative(dldy,y).doit()]])
#print(dldx)
#print(dldy)
#print(L2[0][1])


def func (X,Y,ix,iy,lam):
    ...
    dl0 = np.array([[L2[0][0].subs(x,x0).subs(y,y0).subs(lb,lb0),L2[0][1].subs(x,x0).subs(y,y0).subs(lb,lb0)],[L2[1][0].subs(x,x0).subs(y,y0).subs(lb,lb0),L2[1][1].subs(x,x0).subs(y,y0).subs(lb,lb0)]])
    #print(dl0)
    ...
    ff = ff = fx0 + np.dot(df,dX) + 0.5*fl2
    gg = gx0 + np.dot(dg,dX)
